...
.. code-block:: python
    ...
    ra0, dec0, mu_ra, mu_dec, acc2_ra, acc2_dec = fitter.fit(ra_vs_epoch, dec_vs_epoch, fit_degree=2)

where acc2_ra and acc2_dec are 2 times the acceleration in right ascension and declination, respectively. This factor of 2
comes from the fact that HTOF uses a power series as the basis for the fit. If fit_degree = 3, then
the last two parameters would be six times the jerk in right ascension and declination, respectively.
...
the built-in parallax motions generated by HTOF.
